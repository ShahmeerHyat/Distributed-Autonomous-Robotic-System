...
Measures every metric that appears in the paper comparison tables:
  ...
  M7.  ARIMA evaluator overhead    — timed update_shares() per block (µs)
...
class BenchmarkCollector:
    ...
    def measure_edge_baseline(self, n_runs: int = 6):
        ...
        reflects pure edge compute with no network involvement.
    ...
    def measure_comm_overhead(self):
        ...
        # Snapshot current allocation from the ARIMA manager.
        alloc_weights = self.orch.arima.current_shares
        ...
        print(f"  (Based on current ARIMA allocation — re-run if shares change)\n")
    ...
    def simulate_worker_failure(
        ...
        """
        Simulate a worker crash by injecting PROBE_FAIL_LATENCY into the ARIMA
        history and observing circuit breaker behaviour.
    ...
    def report(self):
        ...
        print(f"\n{SEP}")
        print("  DISTRIBUTED CLIP BENCHMARK REPORT  (ARIMA-V scheduler)")
        ...
        # ── M7: ARIMA evaluator overhead ──────────────────────────────────
        print("\n┌─ M7: ARIMA Evaluator Overhead  (update_shares per block)")
