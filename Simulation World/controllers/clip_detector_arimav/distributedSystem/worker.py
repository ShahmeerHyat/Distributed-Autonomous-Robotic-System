...
ATTN protocol:
  ...
  each shaped (B, H_slice, S, head_dim) in half precision.
  Returns (B, H_slice, S, head_dim) in half precision.

MLP protocol:
  Receives ("MLP", block_idx, ln_x_mlp, start_n, end_n)
  ln_x_mlp is half precision.
  Returns (B, S, embed_dim) in half precision. Master sums slices and adds fc2.bias once.
"""
...
class CLIPWorker:
    ...
    def compute_attn_from_slices(
        ...
        """
        q, k, v : (B, H_slice, S, head_dim)  — may arrive as half precision
        Returns  : (B, H_slice, S, head_dim)  in half precision
        """
        with torch.no_grad():
