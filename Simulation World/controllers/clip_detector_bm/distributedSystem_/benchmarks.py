...
benchmarks.py  —  Runtime benchmark collector for distributed CLIP inference.
                  Adapted for the AIMD scheduler.
...
─────────────────
  ...
  M7.  AIMD evaluator overhead    — get_allocation() + allocate_heads() per block (µs)
...
class BenchmarkCollector:
    ...
    def __init__(
        ...
        self._edge_baseline_latencies: list[float] = []   # from measure_edge_baseline()
        self._edge_live_latencies: list[float]     = []   # from record_inference() edge-only
    ...
    def measure_comm_overhead(self):
        ...
        alloc_weights = self.orch.evaluator.get_allocation()   # AIMD snapshot
    ...
    def simulate_worker_failure(
        ...
        """
        Simulate a worker crash by injecting high latency into the AIMD
        evaluator and observing circuit breaker behaviour, then measure
        recovery time. No-op in edge-only mode.
        """
        if not self.is_distributed:
    ...
    def report(self):
        ...
        print(f"\n{SEP}")
        print(f"  CLIP BENCHMARK REPORT  (AIMD scheduler) — {mode} Mode")
        ...
        if self.is_distributed:
            if self._comm_bytes_total > 0:
                ...
                _fmt_table(
                    "Table 4: Network Transmission Size  (based on current AIMD allocation)",
                    ["Metric", "Value", "Notes"],
                    trans_rows,
                )
            else:
        ...
        if self._evaluator_overhead_us:
            ...
            _fmt_table(
                "Table 5: Scheduler Overhead  (AIMD get_allocation + allocate_heads per block)",
                ["Metric", "Mean / Value", "Std / Notes", "Min", "Max", "N"],
                [
        ...
        if self.is_distributed and self._share_log:
            ...
            _fmt_table(
                f"Table 8: Worker Allocation Stability  (AIMD EMA, {self._share_block_count} blocks)",
