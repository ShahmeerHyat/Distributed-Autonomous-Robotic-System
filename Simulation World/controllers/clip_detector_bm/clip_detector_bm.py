...
BENCHMARK_AFTER_N_FRAMES = 500   # print benchmark report once after this many frames
...
while robot.step(timestep) != -1:
    step_count   += 1
    bench.tick_frame_start()                                    # ← Metric 2
    ...
    if run_inference:
        ...
        if image is not None:
            with torch.no_grad():
                ...
                #    Applies all 12 transformer blocks
                #    Returns (1, 50, 768)
                t0          = time.perf_counter()               # ← Metric 1
                last_hidden = orch.run_inference(hidden)
                inf_lat     = time.perf_counter() - t0          # ← Metric 1
                ...
                last_logits = cls_score.item() * logit_scale.item()

                # 5. Spatial localization via projected patch-text similarity
                ...
                patch_sim = torch.matmul(p_proj[0], text_emb.t()).squeeze()  # (49,)
            ...
            bench.record_inference(                                 # ← Metrics 1, 3, 5
                latency      = inf_lat,
