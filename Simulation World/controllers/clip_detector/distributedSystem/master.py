...
class MasterOrchestrator:
    ...
    def run_inference(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        current_state = x
        if hasattr(self.model, "embeddings") and hasattr(self.model, "pre_layernorm"): # is CLIP model
            current_state = self.model.embeddings(current_state)
            current_state = self.model.pre_layernorm(current_state)
        ...
        if hasattr(self.model, "post_layernorm"): # clip has a final post-encoder layernorm, torchvision ViT does not
