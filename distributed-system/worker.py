import torch
import argparse
import socket
import torchvision.models as models
from shared_utils import recv_msg, send_msg, get_model_metadata, get_head_weights

class ViTWorker:
    def __init__(self, model_name='vit_b_16', use_gpu=True):
        self.device = torch.device("cuda" if use_gpu and torch.cuda.is_available() else "cpu")
        print(f"Worker initialized on {self.device}")
        self.model = getattr(models, model_name)(weights='DEFAULT').to(self.device).eval()
        self.meta = get_model_metadata(self.model)

    def compute_attn_slice(self, block_idx, x, start_h, end_h):
        if start_h >= end_h: return torch.tensor([])
        with torch.no_grad():
            x = x.to(self.device)
            block = self.model.encoder.layers[block_idx]
            
            # Use Notebook's exact slicing
            head_indices = range(start_h, end_h)
            attn_w = get_head_weights(
                block.self_attention.in_proj_weight, 
                head_indices, 
                self.meta["embed_dim"], 
                self.meta["head_dim"]
            ).to(self.device)
            
            # NO BIAS HERE. Just raw linear projection to be merged by Master.
            print(f"Computed Attention Slice Dimension: {(x @ attn_w.t()).shape}")
            return (x @ attn_w.t()).cpu()

    def compute_mlp_slice(self, block_idx, x, start_n, end_n):
        if start_n >= end_n: return torch.tensor([])
        with torch.no_grad():
            x = x.to(self.device)
            mlp = self.model.encoder.layers[block_idx].mlp
            
            # ONS Logic from Notebook
            w1 = mlp[0].weight[start_n:end_n, :]
            b1 = mlp[0].bias[start_n:end_n]
            w2 = mlp[3].weight[:, start_n:end_n] # Column slice!
            
            h = torch.nn.functional.gelu(x @ w1.t() + b1)
            return (h @ w2.t()).cpu()

def run_worker_client(name, master_ip, port, use_gpu):
    worker_instance = ViTWorker(use_gpu=use_gpu)
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    print(f"Connecting to Master at {master_ip}:{port}...")
    sock.connect((master_ip, port))
    send_msg(sock, ('REGISTER', name))
    print(f"Registered with Master as '{name}'. Waiting for tasks...")
    
    try:
        while True:
            msg = recv_msg(sock)
            if msg is None: break
            
            print(f"Received Message: {msg[0]} for block {msg[1]} (slice {msg[3]}:{msg[4]})")
            
            task, block_idx, x, start_idx, end_idx = msg
            
            if task == 'QUIT': break
            elif task == 'ATTN':
                send_msg(sock, worker_instance.compute_attn_slice(block_idx, x, start_idx, end_idx))
            elif task == 'MLP':
                send_msg(sock, worker_instance.compute_mlp_slice(block_idx, x, start_idx, end_idx))
    finally:
        sock.close()

if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument('--name', required=True, help="Worker name (e.g., pc_gpu)")
    p.add_argument('--master_ip', required=True)
    p.add_argument('--port', type=int, default=29500)
    p.add_argument('--gpu', action='store_true')
    args = p.parse_args()
    run_worker_client(args.name, args.master_ip, args.port, args.gpu)